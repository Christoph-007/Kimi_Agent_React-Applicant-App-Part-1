...
export function JobsPage() {
  ...
  // Handle automatic redirect to native applicant view if public route accessed while logged in
  useEffect(() => {
    if (isAuthenticated && user?.type === 'applicant' && window.location.pathname === '/jobs') {
      navigate('/applicant/jobs' + window.location.search, { replace: true });
    }
            ...
            >
              ...
              <div className="flex gap-4">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    const path = user?.type === 'applicant' ? `/applicant/jobs/${job._id}` : `/jobs/${job._id}`;
                    navigate(path);
                  }}
                  className={cn(
                ...
                {(!user || user.type === 'applicant') && (
                  <button
                    onClick={(e) => {
                      ...
                      const path = user?.type === 'applicant' ? `/applicant/jobs/${job._id}` : `/jobs/${job._id}`;
                      if (!isAuthenticated) {
                        navigate('/login', { state: { from: `${path}?apply=true` } });
                        toast.info('Please sign in to apply for this job');
                      } else {
                        navigate(`${path}?apply=true`);
                      }
                    }}
                    className={cn(
                      "flex-1 py-4 rounded-2xl font-bold text-base shadow-sm transition-all duration-300",
