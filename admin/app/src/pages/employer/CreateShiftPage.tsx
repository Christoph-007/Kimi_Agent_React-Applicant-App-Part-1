...
export function CreateShiftPage() {
  ...
  const fetchJobsAndApplicants = async () => {
    try {
      ...
      // Fetch jobs with accepted applicants
      const jobsResponse = await jobsApi.getEmployerJobs({ status: 'open' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to save shift. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
