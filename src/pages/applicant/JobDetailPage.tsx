...
export function JobDetailPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && user?.type === 'applicant' && id && !window.location.pathname.startsWith('/applicant/')) {
      navigate(`/applicant/jobs/${id}${window.location.search}`, { replace: true });
    }
          ...
          <div className="bg-white rounded-2xl p-6 shadow-card sticky top-24">
            ...
            {job.hasApplied ? (
              <div className="p-4 bg-forest-50 border border-forest-200 rounded-xl text-center">
                ...
                <NavLink
                  to="/applicant/my-jobs?tab=applications"
              ...
              <button
                onClick={() => {
                  if (isAuthenticated) {
                    ...
                    navigate('/login', { state: { from: window.location.pathname + '?apply=true' } });
