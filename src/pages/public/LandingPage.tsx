...
function HeroSection() {
  ...
  const handleSearch = () => {
    const basePath = user?.type === 'applicant' ? '/applicant/jobs' : '/jobs';
    if (searchQuery.trim()) {
    ...
    <section className="relative bg-forest-900 min-h-[600px] flex items-center">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
        ...
        <div className="max-w-3xl mx-auto flex flex-col sm:flex-row gap-4">
          <div className="flex-1 bg-white rounded-full p-2 flex items-center shadow-xl border border-gray-100">
          ...
          <button
            ...
            className="bg-lime-200 text-forest-900 w-14 h-14 sm:w-auto sm:px-8 rounded-full font-bold hover:bg-lime-300 transition-all flex items-center justify-center shadow-lg active:scale-95"
          >
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.label}>
                    <NavLink to={link.to} className="text-gray-500 hover:text-forest-700 transition-colors">
                      {link.label}
                    </NavLink>
                  </li>
                ))}
...
export function LandingPage() {
  ...
  const getJobsPath = () => user?.type === 'applicant' ? '/applicant/jobs' : '/jobs';
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-forest-900/95 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <NavLink to={getDashboardPath()} className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center shadow-sm" style={{ backgroundColor: '#C8F435' }}>
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <NavLink to={getJobsPath()} className="text-gray-300 hover:text-white transition-colors">Find Jobs</NavLink>
              <NavLink to="/candidates" className="text-gray-300 hover:text-white transition-colors">Find Candidates</NavLink>
              <NavLink to="/blog" className="text-gray-300 hover:text-white transition-colors">Blog</NavLink>
