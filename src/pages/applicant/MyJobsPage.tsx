...
export function MyJobsPage() {
  ...
  const initialTab = (['active', 'applications', 'requests', 'saved'].includes(tabParam)) ? tabParam : 'active';
  
  const [activeTab, setActiveTab] = useState<'active' | 'applications' | 'requests' | 'saved'>(initialTab);
  ...
  useEffect(() => {
    if (tabParam && ['active', 'applications', 'requests', 'saved'].includes(tabParam) && tabParam !== activeTab) {
                  ...
                  <div key={item._id} className="bg-white rounded-2xl p-6 shadow-card border-l-4 border-forest-600 hover:shadow-card-hover transition-all">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                      ...
                      <div className="flex items-center gap-3">
                        <NavLink
                          to={!isJobRequest ? `/applicant/jobs/${(item as Application).job._id}` : '#'}
