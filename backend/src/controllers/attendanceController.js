...
const checkIn = async (req, res) => {
    try {
        ...
        // Check if shift date is today (with ±1 day tolerance for timezone differences)
        const now = new Date();
        const shiftDate = new Date(shift.date);
        const diffMs = Math.abs(now.setHours(0,0,0,0) - shiftDate.setHours(0,0,0,0));
        ...
        const records = await Attendance.find(query)
            .populate({
                ...
                populate: [
                    ...
                    { path: 'applicant', select: 'name phone email' }
                ]
            })
            .populate('job', 'title')
            .populate('applicant', 'name phone email')
    ...
    try {
        ...
        const records = await Attendance.find(query)
            .populate({
                ...
                populate: [
                    { path: 'job', select: 'title' },
                    { path: 'employer', select: 'storeName' }
                ]
            })
            .populate('job', 'title jobType')
            .populate('employer', 'storeName')
...
// @desc    Decline attendance
// @route   PUT /api/attendance/:id/decline
// @access  Private (Employer)
const declineAttendance = async (req, res) => {
    try {
        ...
        if (attendance.employer.toString() !== req.user._id.toString()) {
            return errorResponse(res, 403, 'Not authorized to decline this attendance');
        }

        if (attendance.isApproved) {
            return errorResponse(res, 400, 'Cannot decline an already approved attendance record');
        }

        if (attendance.isDeclined) {
            return errorResponse(res, 400, 'Attendance is already declined');
        }

        if (!attendance.checkIn || !attendance.checkIn.time) {
            return errorResponse(res, 400, 'Cannot decline an attendance record with no check-in');
        }

        attendance.isDeclined = true;
        attendance.declinedBy = req.user._id;
        attendance.declinedAt = new Date();
        if (declineReason) {
            attendance.declineReason = declineReason;
        }

        await attendance.save();

        return successResponse(res, 200, 'Attendance declined successfully', attendance);
    } catch (error) {
        console.error('Decline attendance error:', error);
        return errorResponse(res, 500, 'Error declining attendance', error.message);
    }
};
...
const markManualAttendance = async (req, res) => {
    try {
        ...
        // Calculate check-in status (late/early/on-time) if time provided
        if (checkInTime) {
            attendance.checkIfLate(shift.startTime);
        }
