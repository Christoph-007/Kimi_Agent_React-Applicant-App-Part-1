    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeFilter === 'unread'
            ? 'bg-forest-900 text-white'
            : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
        >
          ...
          {notifications.map((notification) => {
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl p-6 shadow-card transition-all ${notification.isRead ? 'opacity-75' : ''
                  }`}
              >
                    ...
                    {notification.data && (
                      <div className="mt-4 flex gap-2">
                        {notification.type === 'application' && (
                          <NavLink
                            to={user?.type === 'employer'
                              ? `/employer/jobs/${(notification.data as { jobId: string }).jobId}/applications`
                              : '/applicant/my-jobs?tab=applications'
                            }
                        ...
                        {notification.type === 'shift' && (notification.data as { shiftId?: string }).shiftId && (
                          <NavLink
                            to={user?.type === 'employer'
                              ? '/employer/shifts'
                              : `/applicant/shifts/${(notification.data as { shiftId: string }).shiftId}`
                            }
                            className="px-4 py-2 bg-forest-50 text-forest-700 rounded-lg text-sm font-medium hover:bg-forest-100 transition-colors"
                          >
                            View Shift
                        ...
                        {notification.type === 'job_request' && (
                          <NavLink
                            to={user?.type === 'employer'
                              ? '/employer/requests'
                              : '/applicant/my-jobs?tab=requests'
                            }
                            className="px-4 py-2 bg-forest-50 text-forest-700 rounded-lg text-sm font-medium hover:bg-forest-100 transition-colors"
                          >
                            View Request
                        ...
                        {notification.type === 'attendance' && user?.type === 'employer' && (
                          <NavLink
